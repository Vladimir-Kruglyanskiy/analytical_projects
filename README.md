## Структура репозитория

### 📁 [`darkwood`](https://github.com/Vladimir-Kruglyanskiy/analytical_projects/tree/main/darkwood)

**Проект:** Онлайн-игра (Анализ внутриигровых покупок)

**Инструменты:** SQL (PostgreSQL)

**Краткое описание:** Анализ влияния характеристик игроков и их персонажей на покупку внутриигровой валюты «райские лепестки». Оценка активности платящих и неплатящих игроков.

**Содержимое папки:**

| Файл | Описание |
|------|----------|
| `Secrets_Darkwood.sql` | SQL-запросы для анализа: доля платящих игроков, статистика покупок, активность в разрезе рас, популярные предметы |
| `README.md` | Детальное описание проекта, ключевые выводы и рекомендации |

---

### 📁 [`real_estate`](https://github.com/Vladimir-Kruglyanskiy/analytical_projects/tree/main/estate_market)

**Проект:** Рынок недвижимости Санкт-Петербурга и Ленинградской области

**Инструменты:** SQL (PostgreSQL)

**Краткое описание:** Исследование времени активности объявлений, сезонности публикаций и продаж, сравнительная оценка сегментов Санкт-Петербурга и Ленинградской области для выработки стратегии выхода на рынок.

**Содержимое папки:**

| Файл | Описание |
|------|----------|
| `estate market analysis.sql` | SQL-запросы для анализа: сегментация по времени активности, сезонность, анализ населённых пунктов Ленобласти |
| `README.md` | Детальное описание проекта, ключевые выводы и рекомендации |

---

### 📁 [`investments and startups`](https://github.com/Vladimir-Kruglyanskiy/analytical_projects/tree/main/investments%20and%20startups)

**Проект:** Исследование рынка венчурных инвестиций

**Инструменты:** Python (pandas, matplotlib, seaborn), Jupyter Notebook

**Краткое описание:** Анализ мирового рынка венчурных инвестиций: динамика финансирования стартапов, распределение по отраслям и категориям, факторы, влияющие на успешность стартапов.

**Содержимое папки:**

| Файл | Описание |
|------|----------|
| `investments.ipynb` | Jupyter Notebook с полным анализом: загрузка и предобработка данных, исследовательский анализ, визуализации, выводы |
| `README.md` | Детальное описание проекта, ключевые выводы и рекомендации |

---
### 📁 [`marketplace`](https://github.com/Vladimir-Kruglyanskiy/analytical_projects/tree/main/marketplace)

**Проект:** Анализ маркетплейса

**Инструменты:** Python (pandas, matplotlib, seaborn, numpy), Jupyter Notebook

**Краткое описание:** Системный продуктовый анализ маркетплейса: оценка ключевых метрик, юнит-экономики, поиск точек роста и проверка гипотез через A/B-тестирование.

**Содержимое папки:**

| Файл | Описание |
|------|----------|
| `marketplace.ipynb` | Jupyter Notebook с полным анализом: загрузка и предобработка данных, исследовательский анализ, визуализации, выводы, A/B-тестирование |
| `README.md` | Детальное описание проекта, ключевые выводы и рекомендации |

---

### 📁 [`food delivery`](https://github.com/Vladimir-Kruglyanskiy/analytical_projects/tree/main/food%20delivery)

**Проект:** Аналитика для сервиса доставки еды

**Инструменты:** SQL, Python (pandas, matplotlib, seaborn, scikit-learn), Yandex DataLens, Jupyter Notebook

**Краткое описание:** Комплексный анализ сервиса доставки еды: разработка дашборда с ключевыми бизнес-метриками, исследование пользовательского поведения через воронки, RFM-сегментация пользователей и кластеризация ресторанов для поиска точек роста.

**Содержимое папки:**

| Файл | Описание |
|------|----------|
| [`dashboard`](https://github.com/Vladimir-Kruglyanskiy/analytical_projects/tree/main/food%20delivery) | Дашборд «Ключевые бизнес-метрики в Саранске»: SQL-запросы, визуализации в Yandex DataLens |
| [`insights`](https://github.com/Vladimir-Kruglyanskiy/analytical_projects/tree/main/food%20delivery/insights) | Поиск инсайтов и точек роста: воронки, RFM-сегментация, кластеризация ресторанов |
| `README.md` | Общее описание проектов по сервису доставки |

---

### 📁 [`app`](https://github.com/Vladimir-Kruglyanskiy/analytical_projects/tree/main/app)

**Проект:** Анализ юнит-экономики развлекательного приложения

**Инструменты:** Python (pandas, matplotlib, seaborn), Jupyter Notebook

**Краткое описание:** Анализ юнит-экономики развлекательного приложения после запуска спортивного контента. Выявлены причины стагнации выручки, определены прибыльные и убыточные каналы привлечения, даны рекомендации по оптимизации маркетинговых бюджетов.

**Содержимое папки:**

| Файл | Описание |
|------|----------|
| `app and unit economics.ipynb` | Jupyter Notebook с полным анализом: загрузка и предобработка данных, когортный анализ, расчет CAC, LTV, ROI, анализ каналов, период окупаемости |
| `README.md` | Детальное описание проекта, ключевые выводы и рекомендации |

---

### 📁 [`sollmafin`](https://github.com/Vladimir-Kruglyanskiy/analytical_projects/tree/main/SollmaFin)

**Проект:** Анализ A/B-теста: влияние нового онбординга в SollmaFin

**Инструменты:** Python (pandas, numpy, matplotlib, scipy, statsmodels), Jupyter Notebook

**Краткое описание:** Оценка эффективности обновленного онбординга в трейдинговом приложении. Проверка бизнес-гипотез через A/B-тест, анализ исторических данных, исследование влияния на поведение пользователей.

**Содержимое папки:**

| Файл | Описание |
|------|----------|
| `sollmafin_analysis.ipynb` | Jupyter Notebook с полным анализом: исторические данные, A/B-тест, проверка гипотез, бутстрап-анализ, выводы |
| `README.md` | Детальное описание проекта, ключевые выводы и рекомендации |

---

### 📁 [`books`](https://github.com/Vladimir-Kruglyanskiy/analytical_projects/tree/main/Books%20(ClickHouse))

**Проект:** Анализ пользовательской активности в сервисе электронных книг

**Инструменты:** SQL (ClickHouse)

**Краткое описание:** Исследование пользовательской активности в сервисе Букмейт: анализ поведения пользователей на мобильных платформах и веб-версии, оценка популярности контента, сезонности потребления и технических характеристик приложений.

**Содержимое папки:**

| Файл | Описание |
|------|----------|
| `ClickHouse Books.sql` | SQL-запросы для анализа: топ-20 городов, топ-5 книг, топ-10 авторов, популярность контента в разрезе платформ, сезонность, версионность приложений, тегирование контента, поиск аномалий |
| `README.md` | Детальное описание проекта, ключевые выводы и рекомендации |

---


*Другие проекты будут добавлены позже*




